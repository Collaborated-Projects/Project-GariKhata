...
function sewagelines()
{
	...
	for(i=0; i<55; i++)
	{
		...
		{
			array_circles_sewagelines[i].color(55);
			tip = new Tooltip("circle" + i, mouseX, mouseY, 100, 40);
			tip.show();
		}
		else
		{
			array_circles_sewagelines[i].color(135);
		}
	}
	...
	for(i=0; i<6; i++)
	{
		...
		for(j=0; j<array_polylines_sewagelines[i].array_of_vectors.length - 1; j++)
		{
			...
			{
				array_polylines_sewagelines[i].color(150);
				tip = new Tooltip("polyline" + i, mouseX, mouseY, 100, 40);
				tip.show();
			}
			else
			{
				array_polylines_sewagelines[i].color(0);
			}
		}
	}
	...
	for(i=0; i<9; i++)
	{
		...
		{
			array_polygons_sewagelines[i].color(55);
			tip = new Tooltip("polygon" + i, mouseX, mouseY, 100, 40);
			tip.show();
		}
		else
		{
			array_polygons_sewagelines[i].color(135);
		}
	}
}
