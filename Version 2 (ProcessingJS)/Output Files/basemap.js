...
//polyline
var array_polylines_basemap = [];
var pointer_polylines_basemap = null;
...
function basemapData()
{
	...
	pointer_polylines_basemap = new PolyLine(polyline0);
	array_polylines_basemap[0] = pointer_polylines_basemap;

	var polyline1 = [createVector(815, 573), createVector(805, 574), createVector(541, 593), createVector(32, 617), createVector(17, 617)];
	pointer_polylines_basemap = new PolyLine(polyline1);
	array_polylines_basemap[1] = pointer_polylines_basemap;

	var polyline2 = [createVector(815, 446), createVector(521, 452), createVector(487, 452), createVector(14, 477), createVector(1, 477)];
	pointer_polylines_basemap = new PolyLine(polyline2);
	array_polylines_basemap[2] = pointer_polylines_basemap;
	...
	pointer_polylines_basemap = new PolyLine(polyline3);
	array_polylines_basemap[3] = pointer_polylines_basemap;

	var polyline4 = [createVector(521, 171), createVector(654, 163), createVector(815, 167)];
	pointer_polylines_basemap = new PolyLine(polyline4);
	array_polylines_basemap[4] = pointer_polylines_basemap;
...
function basemap()
{
	...
	{
		var j=0;
		for(j=0; j<array_polylines_basemap[i].array_of_vectors.length - 1; j++)
		{
			if(collidePointLine(mouseX, mouseY, array_polylines_basemap[i].array_of_vectors[j].x, array_polylines_basemap[i].array_of_vectors[j].y, array_polylines_basemap[i].array_of_vectors[j+1].x, array_polylines_basemap[i].array_of_vectors[j+1].y, 1) == true)
			{
				array_polylines_basemap[i].color(0, 0, 0, 255);
